  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cities = Object.entries(cityCounts).map(([name, places]) => ({ name, places }));
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', overflow: 'hidden', zIndex: 0 }}>
      ...
      <div className="top-right-ui">
        <button className="profile-button" onClick={() => setProfileOpen((p) => !p)}>👤</button>
      ...
      {profileOpen && <ProfileModal onClose={() => setProfileOpen(false)} friends={userFriends} cities={visitedCities} />}
          ...
          {isOpen && (
            <VibeSelector
              onSelect={async (emoji: string) => {
                ...
                if (userLocation) {
                  const city = await getCityFromCoords(userLocation[0], userLocation[1]);
